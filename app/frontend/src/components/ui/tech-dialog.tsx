...
const TechDialog: React.FC<TechDialogProps> = ({ isOpen, onClose }) => {
    ...
    return (
        <Dialog open={isOpen} onOpenChange={onClose}>
            <DialogContent className="glass max-h-[80vh] max-w-4xl overflow-y-auto border-white/20">
                <DialogHeader>
                ...
                <div className="glass-card mt-6 rounded-xl p-4 text-center">
                    <div className="mb-2 flex items-center justify-center gap-2">
                        <Cpu className="h-5 w-5 text-green-400" />
                        <span className="font-medium text-white">{t("tech.architecture")}</span>
                    </div>
                    <p className="text-sm text-white/80">{t("tech.architectureBody")}</p>
                </div>

                <button
                    onClick={onClose}
                    className="glass-card absolute right-4 top-4 rounded-full p-2 transition-colors hover:bg-white/10"
